    ...
    def Version():
        ...
        for i in range(len(FITS)):
            ...
            for j in range(len(hdul)-hdul_index):
                ...
                if header['NAXIS'] == 2:
                    if header['BITPIX'] < 0:
                        name_label = f'-float{header["BITPIX"] * -1}-1-{header["NAXIS2"]}-{header["NAXIS1"]}'
                        type_name = f'float{header["BITPIX"] * -1}'
                    if header['BITPIX'] > 0:
                        name_label = f'-uint{header["BITPIX"]}-1-{header["NAXIS2"]}-{header["NAXIS1"]}'
                        type_name = f'uint{header["BITPIX"]}'
                    data = np.expand_dims(data, axis=2)
                elif header['NAXIS'] == 3:
                    if header['BITPIX'] < 0:
                        name_label = f'-float{header["BITPIX"] * -1}-{header["NAXIS3"]}-{header["NAXIS2"]}-{header["NAXIS1"]}'
                        type_name = f'float{header["BITPIX"] * -1}'
                    if header['BITPIX'] > 0:
                        name_label = f'-uint{header["BITPIX"]}-{header["NAXIS3"]}-{header["NAXIS2"]}-{header["NAXIS1"]}'
                        type_name = f'uint{header["BITPIX"]}'
                else:
