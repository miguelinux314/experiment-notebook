...
class MCALIC_Magli(icompression.NearLosslessCodec, icompression.WrapperCodec):
    ...
    def __init__(self, max_error=0, bin_dir=None, data_format=None):
        """
    ...
    def compress(self, original_path: str, compressed_path: str, original_file_info=None):
        ...
        if original_file_info["width"] <= self.max_dimension_size and original_file_info["height"] <= self.max_dimension_size:
    ...
    def compress_one(self, original_path: str, compressed_path: str, original_file_info=None):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            if original_file_info["signed"]:
                offset, original_max = img.min(), img.max()
                offset = min(offset, 0)
            ...
            if original_file_info["signed"]:
                with tempfile.NamedTemporaryFile(dir=options.base_tmp_dir,
                    ...
                    si_file.flush()
                    super().compress(original_path=bil_le_file.name, compressed_path=tmp_compressed_file.name,
                                     original_file_info=original_file_info)
                    tarlite.TarliteWriter(initial_input_paths=[si_file.name, tmp_compressed_file.name]).write(
                        compressed_path)
            else:
                super().compress(original_path=bil_le_file.name, compressed_path=compressed_path,
                                 original_file_info=original_file_info)
            ...
            with tempfile.TemporaryDirectory(dir=options.base_tmp_dir) as tmp_dir:
                ...
                for y in range(self.split_height_count):
                    for x in range(self.split_width_count):
                        ...
                        self.decompress_one(compressed_path=small_compressed_path, reconstructed_path=small_path,
                                            original_file_info=small_file_info)
        ...
        with tempfile.NamedTemporaryFile(
            ...
            if original_file_info["signed"]:
                ...
                with tempfile.TemporaryDirectory(dir=options.base_tmp_dir) as tmp_extract_dir:
                    ...
                    self.decompress_short_names(compressed_path=inner_compressed_path, reconstructed_path=bil_le_file.name, original_file_info=original_file_info)
            else:
                self.decompress_short_names(compressed_path=compressed_path, reconstructed_path=bil_le_file.name,
                                            original_file_info=original_file_info)
    ...
    def decompress_short_names(self, compressed_path, reconstructed_path, original_file_info):
        ...
        try:
            ...
            super().decompress(compressed_path=c_path,
