...
class JPEG_XL(icompression.NearLosslessCodec, icompression.WrapperCodec):
    max_dimension_size = 65535

    def __init__(self, quality_0_to_100=100, compression_level=7, bin_dir=None, output_invocation_dir=None):
        """
        ...
        jpeg_comp_bin_path = os.path.join(bin_dir, "cjxl")
        jpeg_decomp_bin_path = os.path.join(bin_dir, "djxl")

        param_dict = sortedcontainers.SortedDict()
        quality_0_to_100 = int(quality_0_to_100)
        assert 0 <= quality_0_to_100 <=100
        param_dict["quality_0_to_100"] = int(quality_0_to_100)
        compression_level = int(compression_level)
        assert 3 <= compression_level <= 9
        param_dict["compression_level"] = int(compression_level)
        icompression.WrapperCodec.__init__(
            self, compressor_path=jpeg_comp_bin_path, decompressor_path=jpeg_decomp_bin_path,
