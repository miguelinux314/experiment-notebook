...
class QWrapperCodec(NearLosslessCodec):
    """A codec that uses an external process to compress and decompress but quantizing the image using only shifts.
    """

    def __init__(self, codec, qstep:int=0):
        super().__init__()
        self.codec = codec
        self.qstep = qstep
    ...
    def compress(self, original_path: str, compressed_path: str, original_file_info=None):
        ...
        with tempfile.NamedTemporaryFile() as tmp_file:
            ...
            dtypeVariable += str(original_file_info['bytes_per_sample'])

            original_info = np.frombuffer(raw_data, dtype=dtypeVariable)

            original_info = original_info.reshape(original_file_info['height'], original_file_info['width']).astype(
                f"<u{original_file_info['bytes_per_sample']}")

            image_array = np.copy(original_info)
            image_array = np.right_shift(image_array, self.qstep)
            image_array.astype(f">u{original_file_info['bytes_per_sample']}").tofile(tmp_file.name)

            self.codec.compress(tmp_file.name, compressed_path, original_file_info)

    def decompress(self, compressed_path, reconstructed_path, original_file_info=None):
        self.codec.decompress(compressed_path, reconstructed_path, original_file_info)

        with open(reconstructed_path, 'rb') as file:
            raw_data = file.read()
        ...
        original_info = original_info.reshape(original_file_info['height'], original_file_info['width']).astype(f"<u{original_file_info['bytes_per_sample']}")

        image_array = np.copy(original_info)
        image_array= np.left_shift(image_array, self.qstep)
        image_array.astype(f">u{original_file_info['bytes_per_sample']}").tofile(reconstructed_path)
...
class JavaQWrapperCodec(QWrapperCodec):
    """Wrapper for `*.jar` codecs. The compression and decompression parameters are those that need to be
    passed to the 'java' command.
    ...
    def __init__(self, compressor_jar, decompressor_jar, param_dict=None, qstep=0):
        assert shutil.which("java") is not None, \
            f"The 'java' program was not found in the path, but is required by {self.__class__.__name__}. " \
            f"Please (re)install a JRE in the path and try again."
        super().__init__(compressor_path=shutil.which("java"),
