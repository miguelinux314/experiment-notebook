...
class SSIM(CompressionExperiment):
    @atable.column_function("ms_ssim", label="MS-SSIM", plot_min=0)
    def set_MultiScaleSSIM(self, index, row):
        ...
        im1, im2 = [x.astype(np.float64) for x in [original_array, reconstructed_array]]
        mssim = np.array([])
        mcs = np.array([])
        ...
        row[_column_name] = (np.prod(mcs[0:levels - 1] ** weights[0:levels - 1]) *
