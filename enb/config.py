...
class ExistingDirAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        target_dir = values
        try:
            self.assert_valid_dir(target_dir)
...
class ReadableDirAction(ExistingDirAction):
    ...
    def assert_valid_dir(cls, target_dir):
        """Assert that target_dir is a readable dir
        """
        super().assert_valid_dir(target_dir)
...
class ExistingOrCreableDirAction(ExistingDirAction):
    ...
    def assert_valid_dir(cls, target_dir):
        ...
        """
        try:
            ReadableDirAction.assert_valid_dir(target_dir)
            return
        except AssertionError:
            parent_dir = os.path.dirname(target_dir)
            WritableDirAction.assert_valid_dir(parent_dir)
...
def get_options(allow_required=False):
    ...
    execution_options = parser.add_argument_group(
        "Execution options", "General options to control execution modes")
    execution_options.add_argument("-q", "--quick", help="Be quick? Retrieve a small subset of all files when requested for all.",
    ...
    dir_options = parser.add_argument_group("Data dirs", "Data dir configuration")
    # Data dir
    ...
    dir_options.add_argument("--base_dataset_dir", "-d", help=f"Base dir for dataset folders.",
                        ...
                        required=allow_required and not ReadableDirAction.check_valid_dir(default_base_dataset_dir),
    ...
    for default_tmp_dir in ["/dev/shm", "/var/run", tempfile.gettempdir()]:
        try:
            WritableDirAction.assert_valid_dir(default_tmp_dir)
            break
        except AssertionError:
            pass
    else:
