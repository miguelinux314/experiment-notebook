...
int main(int argc, char **argv) {
  ...
  if (isdigit(argv[1][0])) {
    ...
    fseek(in, 0, SEEK_SET);
      ...
      fb_len=&file_buf[i];
      fb_stop=fb_len;	if (fb_stop>&file_buf[FB_SIZE]) fb_stop=&file_buf[FB_SIZE];
      while (i-->0)	{ if (file_buf[i]>0x7f) ++k; }
      if (k==0) method=1;
      else
      if (file_buf[0]=='M' && file_buf[1]=='Z')
	{ method=2; E8E9_encode(fb_pos, fb_len-8-fb_pos); }
      }
#ifndef WIKI
			  TOLIMIT_10=64;
    ...
    // Encode header: version 8, memory option, file size
    out=fopen(argv[3], "wb");
  ...
  else
  {
    ...
    if (getc(in)!='p' || getc(in)!='Q' || getc(in)!=8)
      quit("Not a lpaq8 file");
    MEM=getc(in);
	...
	{ c=e.decompress(); *p++=c; if(c>0x7f) ++k; }
      if (k==0)	{ if(method!=TEXT) quit("Error"); }
      else
      if (file_buf[0]=='M' && file_buf[1]=='Z')
	{ if(method!=EXE) quit("Error"); }
      fb_pos=p;
