...
class TestiSets(unittest.TestCase):
    ...
    def test_array_load_dump(self):
        ...
        for float_value in [True, False]:
            bytes_per_sample_values = [2, 4, 8] if float_value == True else [1, 2, 4, 8]
            for bytes_per_sample in bytes_per_sample_values:
                for signed in [True, False]:
                    ...
                    for value_factor in value_factors:
                        big_endian_values = [True] if (bytes_per_sample == 1 or float_value == True) else [True, False]
                        for big_endian in big_endian_values:
                            row = dict(
                                width=width, height=height, component_count=component_count, float=float_value,
