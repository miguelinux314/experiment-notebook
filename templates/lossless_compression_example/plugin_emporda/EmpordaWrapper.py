...
class EmpordaWrapper(icompression.WrapperCodec, icompression.LosslessCodec):
	...
	def get_transform_params(self, original_path, transformed_path, original_file_info):
		...
		f" {0 if original_file_info['big_endian'] else 1 } 0 -o {transformed_path}.raw -og {original_file_info['component_count']} " \
		f"{original_file_info['height']} {original_file_info['width']} 2 0 0"

	def get_detransform_params(self, reconstructed_path, transformed_path, original_file_info):
		return f"-i {transformed_path}.raw -ig {original_file_info['component_count']} "\
	...
	def get_compression_params(self, original_path, compressed_path, original_file_info):
		return f"-c -i {original_path}.raw -o {compressed_path} -ig {original_file_info['component_count']} " \
	...
	def compress(self, original_path: str, compressed_path: str, original_file_info=None):
		print("\nCOMPRESSION")
		transformed_path = tempfile.NamedTemporaryFile().name #TODO: Possible error
		transform_params = self.get_transform_params(original_path = original_path, transformed_path = transformed_path, original_file_info = original_file_info)
		compression_params = self.get_compression_params(original_path=transformed_path, compressed_path=compressed_path, original_file_info=original_file_info)
		invocation = f"java -jar -Xmx4096m {self.transformer_path} {transform_params}; java -Xmx24g -jar {self.compressor_path} {compression_params}"
		print(invocation)
		try:
			#print("STATUS ", status, "OUTPUT ", output, "MEASURED TIME ", measured_time)
	...
	def decompress(self, compressed_path, reconstructed_path, original_file_info=None):
		print("\nDECOMPRESSION")
		temporal_path = tempfile.NamedTemporaryFile().name
		decompression_params = self.get_decompression_params(compressed_path=compressed_path, reconstructed_path=temporal_path, original_file_info=original_file_info)
		detransform_params = self.get_detransform_params(reconstructed_path=reconstructed_path, transformed_path=temporal_path, original_file_info=original_file_info)
		invocation = f"java -Xmx24g -jar {self.decompressor_path} {decompression_params}; java -jar -Xmx4096m {self.transformer_path} {detransform_params}"
		print(invocation)
